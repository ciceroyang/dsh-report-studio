...
import { markdownToHtml, wrapHtmlDocument, receiptToHtml } from './html.js'
...
export function saveReport(input) {
  ...
  if (input.format === 'html') {
    const title = input.title ?? '工作交付报告'
    const fileContent = wrapHtmlDocument(title, markdownToHtml(input.content), receiptToHtml(receipt), input.content)
    writeFileSync(target, fileContent, 'utf8')
  } else {
    writeFileSync(target, input.content + receipt, 'utf8')
  }
