 ...
 */
const SOURCE_ID = 'id="report-source">'
...
export function parseReceipt(full) {
  ...
  // HTML exports embed the original Markdown in a hidden source block; hash
  // THAT, not the rendered HTML, so md and html copies share one report hash.
  let body
  const srcOpen = full.indexOf(SOURCE_ID)
  if (srcOpen >= 0) {
    const start = srcOpen + SOURCE_ID.length
    const end = full.indexOf('</script>', start)
    body = end >= 0 ? unescapeHtml(full.slice(start, end)) : ''
  } else {
    ...
    body = cut >= 0 ? full.slice(0, cut) : full.slice(0, sepIdx)
  }
...
function pickField(receipt, zhLabel, enLabel) {
  for (const line of receipt.split('\n')) {
    if (!line.includes(zhLabel) && !line.includes(enLabel)) continue
    // The receipt is a two-column table whose first cell is the label, so the
    // value is the first cell that is not the label. The old single-regex form
    // matched `| label |` and returned the label text itself.
    const cells = line.split('|').map((c) => c.trim()).filter((c) => c !== '')
    for (const cell of cells) {
      const text = cell.replace(/<[^>]*>/g, '').trim()
      if (text === '' || text.includes(zhLabel) || text.includes(enLabel)) continue
      return text
    }
  }
  return null
